...
torch::Tensor SAG_cuda(
    ...
    // printf("dimWorker: %d\n", dimWorker);
	// 定义宏在性能上可以减小函数调用的开销，预编译器会将宏直接替换为代码或者对象
    // #define PROFILE 200
...
__global__ void SAG_cuda_kernel(
    ...
    int warpId = tid / WARP_SIZE;                             // global warp-id 全局warp id
    int block_warpId = threadIdx.x / WARP_SIZE;               // block warp-id 块内warp的id
    int laneid = threadIdx.x % WARP_SIZE;                     // warp thread-id -- laneid warp内线程的id
    ...
    if (warpId < num_parts){
        ...
        // pindex_base表示每个part开始的线程号
        const int pindex_base = block_warpId * partSize;
        // #pragma unroll
        // 这里相当于给每一个warp分配一个源节点的所有邻居节点，如果没有执行完，则每次增加一个dimworker
        for (int nidx = partBeg + laneid; nidx < partEnd; nidx += dimWorker){
            // printf("1--pindex_base: %d, laneid: %d\n", pindex_base, laneid);
            // 每个block都有自己的共享内存，将数据并行的放入当前block的共享内存中
            // 因为指针中part_pointer中记录的是column_index的中每个part的起始位置（该序号是按序存放的）
            // 减去partbeg是因为每个block有自己的线程编号
            // 这里partial_ids中记录的是每个block中需要处理的节点
...
) 
{
    ...
    // pytorch的宏，用于解决不同数据类型的函数调用，减少代码量
...
std::vector<torch::Tensor> spmm_backward_cuda(
    ...
    // 该方式可以根据d_output的数据类型来动态调用相应的核，不再需要写多个类型的同一个函数
